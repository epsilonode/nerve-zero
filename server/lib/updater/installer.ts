/**
 * Git checkout + bun install + build.
 */
...
/**
 * Run bun install, then build client and server.
 */
export function buildProject(cwd: string): void {
  const steps: [string, string][] = [
    ['bun install', 'bun install failed'],
    ['bun run build', 'Client build failed'],
    ['bun run build:server', 'Server build failed'],
  ];
